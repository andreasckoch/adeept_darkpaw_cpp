...
{
    printf("Usage: %s --profile FILE.json --poses DIR --gait FILE.json [--max-delta-us N]\n", program);
}
...
int main(int argc, char **argv)
{
    ...
    if (profile_path == 0 || poses_dir == 0 || gait_path == 0 || max_delta_microsec <= 0)
    {
        print_usage(argv[0]);
        return 2;
    }
    ...
    {
        fprintf(stderr, "Semantic gait validation failed: %s\n", error.c_str());
        return 1;
    }

    printf("Semantic gait '%s' is valid: %lu samples, %lu frames\n",
